...
import { FaPencilAlt } from 'react-icons/fa'; // Edit icon ke liye
  ...
  return (
    <div className="p-6 font-mono text-sm text-black bg-white">
      ...
      <p className="text-center text-xs italic my-3">
        Please check goods before leaving. Goods once sold are not returnable/exchangeable.
      </p>
      <hr className="my-3 border-dashed border-black" />
      <div className="space-y-1 text-xs">
        ...
        {data.customerName !== "Walk-in Customer" && (
          <>
            ...
            <p className="flex justify-between font-bold"><strong>Grand Total:</strong> <span>PKR {(data.totalAmount + data.previousDue).toFixed(2)}</span></p>
          </>
        )}
        <p className="flex justify-between"><strong>Amount Paid:</strong> <span>PKR {data.amountPaid.toFixed(2)}</span></p>
        <hr className="my-1 border-dashed" />
        {data.customerName !== "Walk-in Customer" ? (
          <p className="flex justify-between text-base font-bold"><strong>New Balance:</strong> <span>PKR {data.newDue.toFixed(2)}</span></p>
        ) : (
          <p className="flex justify-between text-base font-bold"><strong>Change:</strong> <span>PKR {data.change.toFixed(2)}</span></p>
        )}
      </div>
...
function POS({ products, customers, onProcessSale, cart, setCart }) {
  ...
  if (!customers || !products) {
    return <div className="text-center p-10 dark:text-gray-400">Loading POS data...</div>;
  }
  ...
  const handleDownloadImage = () => {
    ...
    html2canvas(receiptElement, { scale: 3, backgroundColor: '#ffffff' })
      .then((canvas) => {
        ...
        link.href = image;
        link.download = `receipt-${receiptData.id}.png`;
        link.click();
        toast.dismiss();
  ...
  return (
    <div className="grid grid-cols-12 gap-4 p-4 h-[calc(100vh-65px)] bg-gray-100 dark:bg-gray-900">
      <div className="col-span-12 lg:col-span-7 bg-white dark:bg-gray-800 p-4 rounded-xl shadow-lg flex flex-col">
        <input type="text" placeholder="Scan or Search Product by Barcode..." onKeyDown={e => { if (e.key === "Enter") { addToCart(e.target.value); e.target.value = ""; } }} className="w-full bg-gray-50 dark:bg-gray-700 dark:text-gray-200 border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-3 text-lg focus:outline-none focus:ring-2 focus:ring-teal-500 mb-4" />
        <div className="flex-grow overflow-y-auto pr-2">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {products.map((product) => (<div key={product.id} onClick={() => addToCart(product.barcode)} className="bg-gray-50 dark:bg-gray-900 p-3 rounded-lg cursor-pointer text-center transition-all duration-200 hover:shadow-md hover:scale-105 border border-gray-200 dark:border-gray-700"><p className="font-semibold text-gray-800 dark:text-gray-100 truncate text-sm">{product.name}</p><p className="text-sm text-teal-600 dark:text-teal-400 font-bold">PKR {product.salePrice.toFixed(2)}</p><p className={`text-xs ${product.quantity <= 5 ? 'text-red-500' : 'text-gray-500 dark:text-gray-400'}`}>Stock: {product.quantity}</p></div>))}
          </div>
        </div>
      </div>
