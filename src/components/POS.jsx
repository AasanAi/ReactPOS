...
function POS({ products, customers, onProcessSale, cart, setCart }) {
  ...
  const handleSale = () => {
    ...
    if (amountPaid > totalAmount && selectedCustomer !== 'walk-in') {
        toast.error("Amount paid cannot be more than the total for a credit customer!");
        return;
    }
        ...
        {receiptData && (
          <div>
            <div className="p-6 font-mono text-sm text-black">
              ...
              <p><strong>Sale ID:</strong> {receiptData.id}</p>
              ...
              <table className="w-full">
                ...
                <tbody>{receiptData.items.map(item => (<tr key={item.barcode}><td>{item.name}</td><td className="text-right">{item.quantity}</td><td className="text-right">{item.price.toFixed(2)}</td><td className="text-right">{(item.price * item.quantity).toFixed(2)}</td></tr>))}</tbody>
              </table>
              <hr className="my-3 border-dashed border-black" />
              <div className="flex justify-end mt-2">
                <div className="w-2/3">
                  <p className="flex justify-between"><strong>Subtotal:</strong> <span>PKR {receiptData.totalAmount.toFixed(2)}</span></p>
                  {/* FIXED: 'tendered' hata diya gaya, sirf 'amountPaid' rakha hai */}
