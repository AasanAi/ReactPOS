...
function Inventory({ products, onAddProduct, onUpdateProduct, onDeleteProduct }) {
  ...
  const handleEditClick = (productToEdit) => {
    setEditingBarcode(productToEdit.barcode);
  ...
  const importFromCSV = (e) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (importedProducts.length === 0) { toast.error("No new products found in file, or barcodes already exist."); return; }
        importedProducts.forEach(prod => onAddProduct(prod));
        toast.success(`${importedProducts.length} products imported successfully!`);
      } catch (error) { toast.error("Failed to import CSV. Check file format."); console.error("CSV Import Error:", error); }
    };
    ...
    <div className="container mx-auto px-6 py-8">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg">
          <h3 className="text-xl font-bold text-gray-700 dark:text-gray-200 mb-4">{editingBarcode ? 'Edit Product' : 'Add New Product'}</h3>
          <div className="space-y-4">
            ...
            <div><label htmlFor="barcode" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Barcode</label><input id="barcode" type="text" value={newProduct.barcode} onChange={(e) => setNewProduct({ ...newProduct, barcode: e.target.value })} className={`mt-1 w-full bg-white dark:bg-gray-700 dark:text-gray-200 border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-teal-500 ${editingBarcode ? 'bg-gray-200 dark:bg-gray-600 cursor-not-allowed' : ''}`} disabled={!!editingBarcode} /></div>
          </div>
          <div className="flex space-x-2 mt-6"><button onClick={handleFormSubmit} className="w-full bg-teal-600 text-white py-2 rounded-lg font-semibold hover:bg-teal-700 transition-colors shadow-md hover:shadow-lg">{editingBarcode ? 'Update Product' : 'Add Product'}</button>{editingBarcode && (<button onClick={handleCancelEdit} className="w-full bg-gray-500 text-white py-2 rounded-lg font-semibold hover:bg-gray-600 transition-colors">Cancel</button>)}</div>
        </div>
        <div className="lg:col-span-2 bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg">
          ...
          <div className="overflow-x-auto max-h-[60vh]"><table className="min-w-full table-auto text-sm"><thead className="bg-gray-100 dark:bg-gray-700 sticky top-0"><tr><th className="px-4 py-3 text-left font-semibold text-gray-600 dark:text-gray-300">Name</th><th className="px-4 py-3 text-left font-semibold text-gray-600 dark:text-gray-300">Sale Price</th><th className="px-4 py-3 text-left font-semibold text-gray-600 dark:text-gray-300">Stock</th><th className="px-4 py-3 text-left font-semibold text-gray-600 dark:text-gray-300">Actions</th></tr></thead><tbody className="divide-y divide-gray-200 dark:divide-gray-700">{filteredProducts.map((product) => (<tr key={product.barcode} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"><td className="px-4 py-3 text-gray-800 dark:text-gray-100">{product.name}</td><td className="px-4 py-3 font-semibold text-gray-800 dark:text-gray-100">PKR {product.salePrice.toFixed(2)}</td><td className="px-4 py-3 dark:text-gray-300">{product.quantity}</td><td className="px-4 py-3 space-x-2"><button onClick={() => handleEditClick(product)} className="bg-blue-500 text-white px-3 py-1 rounded-md text-xs hover:bg-blue-600 transition-colors">Edit</button><button onClick={() => handleDeleteClick(product.barcode)} className="bg-red-500 text-white px-3 py-1 rounded-md text-xs hover:bg-red-600 transition-colors">Delete</button></td></tr>))}</tbody></table></div>
        </div>
      </div>
    </div>
  );
