...
function Header({ activeTab, setActiveTab }) {
  ...
  const handleTabClick = (tab) => { setActiveTab(tab); setIsMenuOpen(false); };
  ...
  return (
    <>
      <header className="bg-white dark:bg-gray-800 shadow-md sticky top-0 z-20">
        <div className="container mx-auto px-4 sm:px-6 py-3 flex justify-between items-center">
          ...
          <div className="hidden md:flex flex-grow justify-center space-x-2">
            {tabs.map(tab => (
              <button key={tab} onClick={() => handleTabClick(tab)} className={`capitalize px-3 py-2 text-sm font-medium rounded-md transition-colors ${activeTab === tab ? 'bg-teal-600 text-white shadow-sm' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>
                {tab}
              </button>
            ))}
          </div>
      ...
      {/* --- MOBILE SIDEBAR MENU (No Change Here) --- */}
      ...
      <div className={`fixed top-0 left-0 h-full w-64 bg-white dark:bg-gray-800 shadow-xl z-40 transform transition-transform duration-300 ease-in-out ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex justify-between items-center p-4 border-b border-gray-200 dark:border-gray-700">
          <h2 className="text-lg font-bold text-teal-600 dark:text-teal-400">Menu</h2>
          <button onClick={() => setIsMenuOpen(false)} className="text-gray-600 dark:text-gray-300"><FiX size={24} /></button>
        </div>
        <nav className="p-4 flex flex-col space-y-2">
          {tabs.map(tab => (<button key={tab} onClick={() => handleTabClick(tab)} className={`w-full text-left capitalize px-3 py-2 font-medium rounded-md transition-colors ${activeTab === tab ? 'bg-teal-600 text-white' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>{tab}</button>))}
        </nav>
      </div>
    </>
  );
